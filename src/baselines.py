...
def simulate_myopic_one_step(
        ...
        tc_c: np.ndarray,
        tc_k: np.ndarray,
        line_search: Iterable[float] = (0.0, 0.25, 0.5, 0.75, 1.0)
) -> dict[str, pd.Series]:
    dates = returns.index
    N = returns.shape[1]
    w = np.zeros(N)
    ...
    for t in range(len(dates) - 1):
        w_star = targets.iloc[t].values.astype(float)
        sigma_t = returns.rolling(20).std().iloc[t].fillna(0.0).astype(float)

        best_alpha, best_obj, best_wprime, best_delta, best_tc = 0.0, np.inf, w, np.zeros(N), 0.0
        for a in line_search:
            w_prime = (1 - a) * w + a * w_star
            w_prime = _project_bounds(w_prime, weight_low, weight_high)
            w_prime = _project_leverage(w_prime, leverage_cap)
            delta = w_prime - w
            delta = _project_turnover(delta, daily_turnover_cap)

            tc = tc_linear_sqrt(delta, tc_c, tc_k, sigma_t)
            te = np.sum((w_prime - w_star) ** 2)
            obj = tc + te_penalty * te
            if obj < best_obj:
                best_alpha, best_obj = a, obj
                best_wprime, best_delta, best_tc = w_prime, delta, tc
        
        w = best_wprime
        r_tp1 = returns.iloc[t + 1].values.astype(float)
        port_ret.append(float(np.dot(w, r_tp1) - best_tc))
        tc_series.append(best_tc)
