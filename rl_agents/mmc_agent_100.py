...
def train_mmc():
    ...
    # history storage aligned with dqn.py for compatibility
    history={
        'episode_rewards': [], 
        'episode_losses': [], 
        'episode_turnovers': [],
    ...
    for i in range(n_episodes):
        ...
        while not done:
            ...
            # update balance
            # info should have the raw return
            raw_ret=info.get('portfolio_return', 0.0)
            current_balance=current_balance*(1.0+raw_ret)
        ...
        # save stats using compatible keys
        history['episode_rewards'].append(total_reward)
        history['episode_losses'].append(loss)
        history['episode_turnovers'].append(total_turnover)
        # Store average tracking error per step to match dqn.py logic if needed, 
        # but dqn.py stores raw list. Wait, dqn.py stores raw list of values?
        # Looking at dqn.py:
        # episode_tracking_errors.append(episode_tracking_error / max(steps, 1))
        # So it stores the AVERAGE TE per episode. I will do the same.
        avg_te = total_tracking_error / max(steps, 1)
        history['episode_tracking_errors'].append(avg_te)
